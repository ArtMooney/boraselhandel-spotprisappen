      ...
      <div v-show="!loader" class="elprisapp-tabs">
        <div v-show="selectedTab === 0">
            ...
            <template v-for="(label, index) of tableLabels">
              ...
              <div class="elpris-column right">
                <div>
                  {{
                    priceList[index]
                      ? (parseFloat(priceList[index].price) / 10)
                          .toFixed(2)
                          .toString()
                      : ""
                  }}
          ...
          <div class="w-layout-grid elpris-grid">
            ...
            <div class="elpris-column right">
              <div>
                {{ (parseFloat(averagePrice) / 10).toFixed(2).toString() }}
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async getPrices(date) {
      ...
      const priceBackup = this.priceData;
      ...
      if (!this.priceData[0]) {
        ...
        const datePart = this.priceData[0].deliveryDateCET;

        if (this.getDateString(date) !== datePart) {
      ...
      if (this.selectPeriod === "2" && priceData[area].status === "Missing") {
        this.statusMessage = this.missingPricesMessage;
        this.messageBox = "flex";
        priceData = priceBackup;
      } else {
        this.statusMessage = "-";
        this.messageBox = "none";
      }

      // console.log("PRICE DATA", JSON.parse(JSON.stringify(priceData)));

      this.priceList = priceData[area].prices;
      this.tableHeading =
        this.formatHeadingDate(priceData[area].deliveryDateCET) +
        this.addedLabel;
      this.averagePrice = priceData[area].averagePrice;
